(ns taoensso.faraday2.tests.main
  ...
  (:import [cognitect.aws.client.protocol Client]
           [java.net URI]
           [java.util Date]
           [org.testcontainers.containers GenericContainer]
           [taoensso.faraday ConditionalCheckFailedException TransactionCanceledException ValidationException]))
...
(deftest client-creation
  (let [opts->tables #(aws/invoke (#'far/db-client %) {:op :ListTables})]
...
(deftest scan-lazy-seq
  (let [num-items 50]
    ...
    (testing "scan-lazy-seq is not eager"
      (let [client (#'far/db-client *client-opts*)
            clientfn (fn [counter]
                       (reify Client
                         (-invoke [_ op-map]
                           (swap! counter inc)
...
(deftest custom-client
  (let [calls (atom 0)
        client* (aws/client (assoc *client-opts* :api :dynamodb))
        client (reify Client
                 (-invoke [_ op-map]
                   (swap! calls inc)
                   (aws/invoke client* op-map)))]
    ...
    (is (= 1 @calls)))
  (let [calls (atom 0)
        client* (aws/client (assoc *client-opts* :api :streams-dynamodb))
        client (reify Client
                 (-invoke [_ op-map]
                   (swap! calls inc)
                   (aws/invoke client* op-map)))]
